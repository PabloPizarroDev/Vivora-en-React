...
class Game extends React.Component {
  ...
  gameTick = () => {
    ...
    var newx = this.state.snakeList[0][0] + newInc[0],
    ...
    if (
      ...
      body.unshift([
        (body[0][0] + this.state.increment[0]) % 19,
        (body[0][1] + this.state.increment[1]) % 19,
      ]);
      if (!(body[0][0] === food[0] && body[0][1] === food[1] && ifs))
        body.pop();
      else ifs = false;
      this.setState({
        snakeList: body,
  ...
  componentDidMount() {
    ...
    keyboard.addEventListener("keydown", (e) => {
      ...
      if (
        ...
        return;
      var newx = this.state.snakeList[0][0] + newInc,
        newy = this.state.snakeList[0][1] + newInc;
      if (
        this.state.snakeList.filter((i) => {
          return i[0] === newx % 19 && i[1] === newy % 19;
        }).length ||
      ...
      <>
        <div class="main-game-container">
          <div class="innergamecontainer">
            ...
            <button className="restar" onClick={this.restartGame}>
              Restart Game
            </button>
          </div>
        </div>
